                        ...
                        {currUrl ? (
                          <div className="aspect-[4/5] rounded-2xl overflow-hidden border border-gray-200 shadow-sm relative group bg-gray-50">
                             <img src={currUrl} alt={`Preview ${idx+1}`} className="w-full h-full object-cover group-hover:opacity-40 transition-all" />
                              <div className="absolute inset-0 m-auto h-10 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                 {item.field === 'image_url' && (
                                   <button type="button" onClick={() => setCropFileContext({ url: currUrl, file: null as any })} className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center shadow-xl hover:bg-[#D4AF37] hover:text-black transition-all" title="Enquadrar">
                                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" /></svg>
                                   </button>
                                 )}
                                 <button type="button" onClick={() => setValue(item.field, '')} className="w-10 h-10 bg-red-500 text-white rounded-full flex items-center justify-center shadow-xl hover:bg-red-600 transition-all" title="Remover">
