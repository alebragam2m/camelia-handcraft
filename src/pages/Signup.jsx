      ...
      <div className="bg-white p-8 md:p-12 rounded-[40px] shadow-2xl w-full max-w-md relative z-10 border border-gray-50">
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-5">
          ...
          <div className="relative">
            <input
              type={showPassword ? 'text' : 'password'} placeholder="Senha (mínimo 6 caracteres)" value={password}
              onChange={(e) => setPassword(e.target.value)} required minLength={6}
              className="w-full p-4 pr-12 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:border-primaria focus:bg-white transition-all font-sans text-sm"
            />
            <button
              type="button"
              onClick={() => setShowPassword(!showPassword)}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-primaria transition-colors"
